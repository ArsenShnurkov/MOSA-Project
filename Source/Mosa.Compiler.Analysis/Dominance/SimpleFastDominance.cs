...
namespace Mosa.Compiler.Analysis.Dominance
{
	/// <summary>
	...
	public sealed class SimpleFastDominance : IDominanceProvider
	{
		...
		/// </summary>
		public SimpleFastDominance(BasicBlock entryBlock)
		{
			// Blocks in reverse post order topology
			IList<BasicBlock> blocks = entryBlock.ReversePostOrder(); //basicBlocks.GetConnectedBlocksStartingAtHead(entryBlock);
		...
		private void CalculateDominance(IList<BasicBlock> reversePostOrder)
		{
			// Changed flag
			bool changed = true;
			...
			while (changed)
			{
				...
				foreach (BasicBlock b in reversePostOrder)
				{
					...
					foreach (var previous in b.Sources)
					{
						if (idom == null)
						{
							idom = previous;
		...
		private void CalculateChildren(IList<BasicBlock> basicBlocks)
		{
		...
		/// </summary>
		private void CalculateDominanceFrontier(IList<BasicBlock> basicBlocks)
		{
			foreach (BasicBlock b in basicBlocks)
			{
				if (b.Sources.Count > 1)
				{
					foreach (BasicBlock p in b.Sources)
					{
						BasicBlock runner = p;
