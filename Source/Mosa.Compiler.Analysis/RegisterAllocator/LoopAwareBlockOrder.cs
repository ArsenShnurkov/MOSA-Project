...
namespace Mosa.Compiler.Analysis.RegisterAllocator
{
	...
	public sealed class LoopAwareBlockOrder
	{
		...
		{
			return loopHeader.Get(block.Sequence);
		}

		public bool IsLoopEnd(BasicBlock block)
		{
			return loopEnds.Contains(block);
		}
		...
		public LoopAwareBlockOrder(IList<BasicBlock> basicBlocks)
		{
		...
		private void CountEdges(BasicBlock current, BasicBlock parent)
		{
			...
			foreach (var next in current.Targets)
			{
		...
		private void MarkLoops()
		{
			...
			{
				var loopStart = loopEnd.Targets[0]; // assuming the first one?
				...
				while (worklist.Count != 0)
				{
					...
					foreach (var previous in current.Sources)
					{
		...
		private void ComputeOrder(BasicBlock start)
		{
			...
			while (workList.Count != 0)
			{
				...
				foreach (BasicBlock successor in block.Targets)
				{
					forwardBranchesCount[successor.Sequence]--;

					if (forwardBranchesCount[successor.Sequence] == 0)
					{
						workList.Add(new Priority(loopDepth[successor.Sequence], successor.Sequence, false), successor);
					}
				}
			}
		}
